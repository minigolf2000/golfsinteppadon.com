define({
    ...
    createHTML: function () {
        this.$el = $('<div id="ball">').css('background-position', '0px 0px');
        this.render();
        return this.$el;
    },
    ...
    render: function () {
        this.$el.css({
            left: this.pos[0] - this.BALL_RADIUS + "px",
            top: this.pos[1] - this.BALL_RADIUS + "px"
        })
        // If the ball's velocity is past a threshold, make the ball look like it is rolling
        if (lengthSquared(this.vel) > 9) {
            this.$el.css('background-position', parseInt(this.$el.css('background-position')) % 112 - 28 + 'px 0px');
        }
    }
});